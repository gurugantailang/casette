...
struct LCDOutputConfig {
  ...
  /// fftLEDOutput()
  void (*update_callback)(LCDOutputConfig *cfg, LCDOutput *matrix) = nullptr;
...
class LEDOutput {
 public:
  ...
  /// led_matrix を更新します (FastLED.show() を呼び出します)
  void display() {
    FastLED.show();
  }

  /// FFTDisplay オブジェクトへのアクセスを提供します
...
/// FFT 結果を「棒グラフ」として提供するデフォルトの更新実装
void fftLEDOutput(LEDOutputConfig *cfg, LEDOutput *matrix) {
  // process horizontal
